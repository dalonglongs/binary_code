...
def build_info(config):
    ...
    for path, dir_list, file_list in group:
        ...
        for i in range(len(file_list)):
            ...
            if is_elf_file(file_path):
                ...
                split_path = os.path.normpath(file_path).split('test')
                # print(split_path)
                output_path = os.path.join(output_dir, split_path[1].replace("/", '', 1))
    ...
    for i in range(len(binary_files)):
        ...
        block_idx_to_tokens, per_block_neighbors_bids, node_dict, cfg = preprocessing(
