...
from graph_match.dataset import build_label, build_dataset
from graph_match.loss import pairwise_loss
from graph_match.evaluation import compute_similarity, auc
from graph_match.graph_embedding_network import GraphEncoder, GraphAggregator, GraphEmbeddingNet
from graph_match.graph_match_network import GraphMatchingNet
...
def build_model(config):
    ...
    if graph_match_conf['model_type'] == 'embedding':
        ...
        raise ValueError('model_type的参数不合法')
...
def get_graph(graph, labels):
    ...
    node_distance = torch.from_numpy(graph.nodes_dis_matrix)
    node_distance = torch.stack([node_distance]).T
    labels = torch.from_numpy(labels).long()
    return node_features, edge_features, from_idx, to_idx, graph_idx, node_distance, labels
...
def execute_train(config):
    ...
    for i_iter in range(graph_match_conf['training']['n_training_steps']):
        ...
        for i in range(0, len(x_train)):
            ...
            # 解包数据
            node_features, edge_features, from_idx, to_idx, graph_idx, node_distance, labels = get_graph(batch_graph, batch_label)

            labels = labels.to(device)
            graph_vectors = model(node_features.to(device), edge_features.to(device), from_idx.to(device),
                                  to_idx.to(device), graph_idx.to(device), node_distance.to(device), len(labels) * 2)
            ...
            if ((i_iter + 1) // graph_match_conf['training']['print_after'] %
                ...
                with torch.no_grad():
                    ...
                    for i in range(0, len(x_test)):
                        batch = x_test[i]
                        label = y_test[i]
                        node_features, edge_features, from_idx, to_idx, graph_idx,  node_distance, labels = get_graph(batch, label)
                        labels = labels.to(device)
                        ...
                        biaoqian.append(labels)

                    pair_auc = auc(torch.cat(similaritys), torch.cat(biaoqian))

                    # eval_metrics = {'pair_auc': np.mean(accumulated_pair_auc)}
                    # info_str += ', ' + ', '.join(['%s %.4f' % ('val/' + k, v) for k, v in eval_metrics.items()])
                    model.train()
                print('iter %d, %s, eval_auc=%s' % (i_iter + 1, info_str, str(pair_auc)))
