...
from graph_match.segment import unsorted_segment_sum
...
class GraphPropLayer(nn.Module):
    ...
    def __init__(self, node_state_dim, edge_hidden_sizes, node_hidden_sizes,
        ...
        self.atti_bias_layer = nn.Sequential(
                                             nn.Linear(1, 3),
                                             nn.Linear(3, 1))
...
class GraphEmbeddingNet(nn.Module):
    ...
    def _apply_layer(self, layer, node_states, from_idx, to_idx, graph_idx, node_distance, n_graphs, edge_features):
        ...
        进行一次传播
        """
        del graph_idx, n_graphs, node_distance
        return layer(node_states, from_idx, to_idx, edge_features=edge_features)

    def forward(self, node_features, edge_features, from_idx, to_idx, graph_idx, node_distance, n_graphs):
        """
        计算图表示
        ...
        """
        # 编码器(1. MLP(node features) -> node state, 2.MLP(edge features) -> edge feature)
        node_features, edge_features = self._encoder(node_features, edge_features)
        node_states = node_features

        # 图传播层(1.边特征 -> message, 2.message + node state -> new node state)
        layer_outputs = [node_states]
        for layer in self._prop_layers:
            node_states = self._apply_layer(layer, node_states, from_idx, to_idx, graph_idx, node_distance, n_graphs, edge_features)
