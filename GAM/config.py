...
def get_default_config():
    ...
    binary_dir = '../../ProjectData/ExperimentData'
    output_dir = '../../ProjectData/Output'
    ...
    word2vec_embedding_dim = 32  # 嵌入的维度

    model_type = 'matching'  # 模型的类型, 可选值{matching, embeddin}
    ...
    return dict(
        ...
        word2vec=dict(
            sequence_length=10,  # 句子的长度
            window_size=2,  # 窗口大小
            num_ns=2,  # 每个正采样对应的负采样数量
            seed=42,
            embedding_dim=word2vec_embedding_dim,  # 嵌入的维度
            num_epochs=200,  # 训练的轮数
            batch_size=50,  # 训练的批的大小
            learning_rate=1e-3,  # 学习速率
        ),
        graph_match=dict(
            seed=8,
            dataset_type=['coreutils', 'diffutils', 'findutils', 'test', 'binutils'],
